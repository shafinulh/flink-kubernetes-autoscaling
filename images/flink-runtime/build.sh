...
if [[ -n "${LOCAL_IMAGE_OVERRIDE}" ]]; then
  FLINK_RUNTIME_LOCAL="${LOCAL_IMAGE_OVERRIDE}"
else
  FLINK_RUNTIME_LOCAL="${FLINK_RUNTIME_IMAGE_NAME}:${FLINK_RUNTIME_IMAGE_TAG}"
fi
...
cmd+=(
  -f "${REPO_ROOT}/images/flink-runtime/Dockerfile"
  --build-arg "FLINK_RUNTIME_SOURCE_DIR=${FLINK_RUNTIME_SOURCE_DIR}"
  -t "${FLINK_RUNTIME_LOCAL}"
  "${REPO_ROOT}"
)

echo "Building ${FLINK_RUNTIME_LOCAL}"
printf 'Command:'
printf ' %q' "${cmd[@]}"
printf '\n'
